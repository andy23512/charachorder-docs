...
**Done:** `ManagerSettingsModifiers.png` ("Chord Modifiers") -- confirmed no
corresponding settings box exists in the current UI at all. It was never a
CCOS setting, just fixed key-location documentation, so there was nothing to
screenshot any more. Deleted the image and the `.. image::` block plus its
one line of caption text from `docs/Device Manager.rst:219-223`; the
...
- [x] [#141](https://github.com/CharaChorder/docs/pull/141) — Shortens the Learning the Layout intro on CC1/CC2/M4G, moving the community-consensus/VIM prose into new `Layout.rst` sections (Layout Philosophy, Community Consensus). Adopted with fixes (commits `640c392`/`6d42763`): the PR copy-pasted "CC English layout" into Master Forge's copy, which calls its layout "M4 English layout" everywhere else, so that was corrected; it also added a "start training on DOT I/O" link that duplicated the existing Practice subsection further down the same section, so that was dropped; and `Layout.rst`'s new intro was reworded to drop the same "CC English layout" naming, since that page is shared by every device.

## Needs reconciliation with this fork's own changes

- [x] [#95](https://github.com/CharaChorder/docs/pull/95) — Large `Master Forge.rst` rewrite from 2025-08-12 (links, pre-order items, Kickstarter bundle count). Hand-applied the verifiable factual fixes instead of merging wholesale (commit `ae0fae7`): nine M2 screw count/locations, bookend rail screw size, switch cap/A2/A3 placeholder text, new switch-name diagrams, A2/A3/Shift sections repointed at this page's own M4 English Layout graphic with the labels actually printed on it (②, ③, ⇧), Toggleable Layers/A1 Layer renamed from "Alpha" (fixing a toggle-back logic error), and Kickstarter Premium/Super tiers corrected to "One (1) Ergo Bolt-On set". Skipped the Emergency firmware-update dropdown reformatting, already-fixed manager.charachorder.com links, and the CAD/VIP pre-order additions (likely misattributed Kickstarter-exclusive perks).
- [x] [#117](https://github.com/CharaChorder/docs/pull/117) / [#119](https://github.com/CharaChorder/docs/pull/119) / [#120](https://github.com/CharaChorder/docs/pull/120) / [#107](https://github.com/CharaChorder/docs/pull/107) — Link fixes for CC1/CCL/CCX/CC2. Adopted (commits `a46e564`/`a82a3e0`/`d344d46`/`8d238ad`, with the CC1/CC2 "Learning the Layout" intro conflict hand-resolved). Confirmed via a full HTML build that CC1 and CC2's identical `power cable`/`TRS cable`/`CCEnglish Layout` explicit labels were a real cross-file bug (not just theoretical): CC1's own "graphic"/cable links were silently resolving to CC2's page. The PRs' device-prefixed label rename (`CC1 Power Cable`, `CC2 CCEnglish Layout`, etc.) fixes this; verified post-merge that all four devices' refs now self-resolve.
- [x] [#118](https://github.com/CharaChorder/docs/pull/118) — CC1 typo fixes. Adopted (commit `c037dbc`): "Head over the the" duplicate word across CC1/CC2/CCX/CCL, CCL's "assembed"→"assembled", and typo fixes in Device Manager/GenerativeTextMenu/Glossary/SerialAPI. Three conflicts (CC2, Device Manager, Glossary) hand-resolved in favor of this fork's current wording/links. The flagged risky hunk (`:doc:`Layout<Layout>`` → `CC English Layout`) did not apply at all, as expected: this fork's line already reads `:ref:`CC English layout<CC1 CCEnglish Layout>`` from earlier-adopted #117/#141 work, so the outdated context simply didn't match. Also caught and fixed an unrelated Sphinx title-underline-too-short warning on `Layout.rst` introduced by a heading-length change in the same PR; build confirmed 0 warnings after.
